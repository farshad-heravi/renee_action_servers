// ArmMotionPlan action server: plans and executes via MoveIt move_group (Pilz OMPL, etc.).
#include <memory>
#include <mutex>
#include <string>
#include <thread>
...
#include "renee_action_servers/action/arm_motion_plan.hpp"

using ArmMotionPlan = renee_action_servers::action::ArmMotionPlan;
using GoalHandleArmMotionPlan = rclcpp_action::ServerGoalHandle<ArmMotionPlan>;

class ArmMotionPlanServer : public rclcpp::Node
{
public:
  ArmMotionPlanServer()
  : rclcpp::Node("arm_motion_plan_server")
  {
    ...
    default_tcp_link_ = this->declare_parameter<std::string>("default_tcp_link", "robot_arm_tool0");
    ...
    action_server_ = rclcpp_action::create_server<ArmMotionPlan>(
      ...
      std::bind(&ArmMotionPlanServer::handle_goal, this, std::placeholders::_1, std::placeholders::_2),
      std::bind(&ArmMotionPlanServer::handle_cancel, this, std::placeholders::_1),
      std::bind(&ArmMotionPlanServer::handle_accepted, this, std::placeholders::_1));

    RCLCPP_INFO(
      get_logger(),
      "ArmMotionPlan server on 'moveit_arm_motion_plan' (MoveIt ns='%s', group='%s')",
...
private:
  rclcpp_action::GoalResponse handle_goal(
    ...
    std::shared_ptr<const ArmMotionPlan::Goal> goal)
  {
  ...
  rclcpp_action::CancelResponse handle_cancel(const std::shared_ptr<GoalHandleArmMotionPlan>)
  {
  ...
  void handle_accepted(const std::shared_ptr<GoalHandleArmMotionPlan> goal_handle)
  {
  ...
  void execute(const std::shared_ptr<GoalHandleArmMotionPlan> goal_handle)
  {
    ...
    moveit::planning_interface::MoveGroupInterface::Options opt(
      planning_group_, moveit::planning_interface::MoveGroupInterface::ROBOT_DESCRIPTION, move_group_ns_);
    ...
    const std::string ee = goal->tcp_link.empty() ? default_tcp_link_ : goal->tcp_link;
    if (!ee.empty()) {
      mgi->setEndEffectorLink(ee);
    }

    const double v = (goal->velocity_scaling > 0.0) ? goal->velocity_scaling : default_velocity_scaling_;
    const double a =
      (goal->acceleration_scaling > 0.0) ? goal->acceleration_scaling : default_acceleration_scaling_;
    mgi->setMaxVelocityScalingFactor(static_cast<double>(v));
    mgi->setMaxAccelerationScalingFactor(static_cast<double>(a));

    mgi->setPoseTarget(goal->target_pose);

    RCLCPP_INFO(
      get_logger(), "Planning [%s / %s] to frame=%s",
  ...
  rclcpp_action::Server<ArmMotionPlan>::SharedPtr action_server_;
  std::string planning_group_;
  std::string move_group_ns_;
  std::string default_tcp_link_;
...
int main(int argc, char ** argv)
{
  ...
  auto node = std::make_shared<ArmMotionPlanServer>();
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}
